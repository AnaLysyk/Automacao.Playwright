...
import { CidadaoSmartAgendamentoDataHoraPageSelectors as DataHoraS } from './pages/selectors/CidadaoSmartAgendamentoDataHoraPageSelectors';
  ...
  test('deve executar agendamento com digitacao humana e validacao de codigo', async ({ page }) => {
   ...
   await digitarComoUsuario(page.getByPlaceholder('Digite...'), dadosPessoa.nome);
    ...
    await test.step('5. Validar codigo por e-mail (sem UI de Gmail)', async () => {
      ...
      const source = ((env.CIDADAO_SMART_SECURITY_CODE_SOURCE as string) || 'env').toLowerCase();
      let codigoSeguranca = '';
      ...
      if (source === 'imap') {
        ...
        await page.pause();
        codigoSeguranca = (env.CIDADAO_SMART_SECURITY_CODE as string) || '';
        test.skip(!codigoSeguranca, 'Defina CIDADAO_SMART_SECURITY_CODE para modo manual/env.');
      }

      await digitarComoUsuario(page.getByLabel(/codigo de seguranca|código de segurança/i), codigoSeguranca);
      await pausaVisual();
