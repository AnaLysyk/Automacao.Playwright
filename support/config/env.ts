...
export type EnvConfig = {
  ...
  apiTokenPath: string;
  apiTokenGrantType: string;
  apiTokenUserName: string;
  apiTokenUserPassword: string;
...
export function loadEnv(): EnvConfig {
  return {
    ...
    smartApiBaseUrl: normalizeUrl(read('SMART_API_BASE_URL', read('API_BASE_URL'))),
    cidadaoSmartApiBaseUrl: normalizeUrl(read('CIDADAO_SMART_API_BASE_URL', read('API_BASE_URL'))),
    bookingApiBaseUrl: normalizeUrl(read('BOOKING_API_BASE_URL', read('API_BASE_URL'))),
    ...
    apiTokenPath: read('API_TOKEN_PATH', '/api/tokens'),
    apiTokenGrantType: read('API_TOKEN_GRANT_TYPE', 'CREDENTIALS'),
    apiTokenUserName: read('API_TOKEN_USER_NAME', read('SMART_USER')),
    apiTokenUserPassword: read('API_TOKEN_USER_PASSWORD', read('SMART_PASSWORD')),

    keycloakTokenUrl: read(
      'KEYCLOAK_TOKEN_URL',
      'https://keycloakdev.griaule.com:9111/realms/dev/protocol/openid-connect/token',
    ),
    keycloakGrantType: read('KEYCLOAK_GRANT_TYPE', 'client_credentials'),
    keycloakClientId: read('KEYCLOAK_CLIENT_ID', 'nkey'),
    keycloakClientSecret: read('KEYCLOAK_CLIENT_SECRET'),
    keycloakUsername: read('KEYCLOAK_USERNAME', 'gbds_bind'),
    keycloakPassword: read('KEYCLOAK_PASSWORD'),
    keycloakResponseType: read('KEYCLOAK_RESPONSE_TYPE', 'code'),
    keycloakScope: read('KEYCLOAK_SCOPE', 'openid'),

    xOperatorCpf: read('X_OPERATOR_CPF'),
    ...
    emailCodeMode: readEnum(
      'EMAIL_CODE_MODE',
      ['manual', 'env', 'browser', 'gmail-api', 'imap', 'internal-api', 'log'],
      'manual',
    ),
    ...
    nomeRequerenteBooking: read('NOME_REQUERENTE_BOOKING', 'Teste Automacao Booking'),
    dataNascimentoRequerente: read('DATA_NASCIMENTO_REQUERENTE', '1990-01-01'),
    cpfComProcessoFinalizado: read('CPF_COM_PROCESSO_FINALIZADO'),

    servicePointId: read('SERVICE_POINT_ID', read('CIDADAO_SMART_DEFAULT_SERVICE_POINT_ID')),
    ...
    bookingCancelAppointmentMethod: read('BOOKING_CANCEL_APPOINTMENT_METHOD', 'DELETE'),
...
export function requireConfiguredUrl(
  name: keyof Pick<EnvConfig, 'cidadaoSmartBaseUrl' | 'bookingBaseUrl'>,
): string {
